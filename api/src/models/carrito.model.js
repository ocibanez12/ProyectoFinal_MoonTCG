...
export async function agregarAlCarrito({ usuario_id, producto_id, cantidad = 1 }) {
  ...
  const { rows } = await consultar(sql, [usuario_id, producto_id, cantidad]);
  return rows[0];
}
...
export async function eliminarDelCarrito({ usuario_id, producto_id }) {
  const { rowCount } = await consultar('DELETE FROM carrito WHERE usuario_id = $1 AND producto_id = $2', [usuario_id, producto_id]);
  return rowCount > 0;
}

export async function vaciarCarrito({ usuario_id }) {
  const { rowCount } = await consultar('DELETE FROM carrito WHERE usuario_id = $1', [usuario_id]);
  return rowCount;
}
